...
class COCOStuffeval:
    ...
    def __init__(self, cocoGt, cocoRes, stuffStartId=0, stuffEndId=255, addOther=True):
        '''
    ...
    def evaluate(self):
        ...
        # @LH: on 11/12/2018, add background category label
        # labelCount = max([c for c in self.cocoGt.cats]) 
        labelCount = 1 + max([c for c in self.cocoGt.cats]) 
        confusion = np.zeros((labelCount, labelCount))
        for i, imgId in enumerate(imgIds):
            if i+1 == 1 or i+1 == len(imgIds) or (i+1) % 10 == 0:
                print('\rEvaluating image %d of %d: %d' % (i+1, len(imgIds), imgId), end='')
    ...
    def _accumulateConfusion(self, cocoGt, cocoRes, confusion, imgId):
        ...
        # @LH: 11/13/2018 +1 to shift class labels from [0,1,2] to [1,2,3]
        validGt = 1 + labelMapGt[valid].astype(int)
        validRes = 1 + labelMapRes[valid].astype(int)
